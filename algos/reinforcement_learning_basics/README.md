...
- $\mathcal{S}$：状态空间，也就是“环境可能处于哪些状态”
- $\mathcal{A}$：动作空间，也就是“agent 能做哪些动作”
- $P$：状态转移概率，表示在状态 $s$ 下执行动作 $a$ 后，环境会以多大概率变到下一个状态 $s'$
- $R$：奖励函数，表示在某个状态或状态转移上，环境给 agent 的反馈是多少
- $\gamma$：折扣因子，控制未来奖励在当前看来值多少钱

如果只背符号，很容易觉得抽象。可以直接用一个“走格子”例子来记：

- $\mathcal{S}$：格子地图上所有位置，比如左、中、右三个格子
- $\mathcal{A}$：`left` 和 `right`
- $P$：如果当前在中间格子，执行 `right` 后，有 0.9 的概率走到右边，0.1 的概率因为打滑还留在原地
- $R$：走到终点格子奖励 `+1`，掉进陷阱格子奖励 `-1`，平时每走一步奖励 `-0.01`
- $\gamma$：如果设成 `0.9`，说明未来奖励仍然重要，但越远越打折
...
比如做自动驾驶时，如果状态里只有“当前方向盘角度”，却没有“当前车速”，那这个状态往往就不够完整，因为同样的方向盘角度，在低速和高速下的后果可能完全不同。也就是说，未来怎么变化不能只看当前给出的这个残缺状态，还得依赖你没喂进去的历史信息。
...
- $G_t$：时刻 $t$ 开始的 return，也就是“从现在往后总共能拿多少钱”
- $k$：往后看第几步，`k = 0` 表示当前这一步，`k = 1` 表示下一步
- $\gamma^k$：第 $k$ 步奖励的折扣权重
- $r_{t+k}$：从当前开始往后第 $k$ 步拿到的单步奖励

可以直接举个数值例子：

- 假设之后三步奖励分别是 `1.0`、`1.0`、`1.0`
- 如果 $\gamma = 1.0$，那么 return 就是 `1 + 1 + 1 = 3`
- 如果 $\gamma = 0.9$，那么 return 就是 `1 + 0.9 + 0.81 = 2.71`

所以：

- `reward` 是单步反馈
- `return` 是从当前时刻往后的累计目标
- `gamma` 越小，模型越偏短视；越接近 `1`，模型越看重长期收益
...
- $\pi$：当前策略，也就是“在每个状态下打算怎么选动作”
- $s$：某个具体状态
- $G_t$：从当前时刻开始的累计回报
- $\mathbb{E}_\pi[\cdot]$：表示“按照策略 $\pi$ 反复执行后，取平均结果”
...
- $a$：当前先执行的那个动作
- $a_t = a$：表示在时刻 $t$ 先把这个动作固定住，再看后续回报
...
- $A^\pi(s, a)$：动作 $a$ 相对当前平均水平到底好多少
- 如果 $A^\pi(s, a) > 0$，说明这个动作比该状态下的平均动作更好
- 如果 $A^\pi(s, a) < 0$，说明这个动作偏差
...
- 当前在中间格子 $s$
- 策略 $\pi$ 平均来说最终能拿到 `0.6` 的回报，那么 $V^\pi(s) = 0.6$
- 如果此时先走 `right`，最终平均能拿到 `0.9`，那么 $Q^\pi(s, \text{right}) = 0.9$
- 如果此时先走 `left`，最终平均只能拿到 `0.2`，那么 $Q^\pi(s, \text{left}) = 0.2$
- 所以 $A^\pi(s, \text{right}) = 0.3$，$A^\pi(s, \text{left}) = -0.4$
...
- $Q^\*(s, a)$：最优策略下，在状态 $s$ 执行动作 $a$ 的真实最优价值
- $r$：当前这一步立刻拿到的奖励
- $\gamma$：折扣因子
- $s'$：执行完动作后的下一状态
- $a'$：下一状态里候选的动作
- $\max_{a'} Q^\*(s', a')$：下一状态里最好的后续价值
- $\mathbb{E}[\cdot \mid s, a]$：因为环境可能有随机性，所以这里取条件期望

可以继续用走格子例子：

- 当前在中间格子 $s$
- 先执行 `right` 这个动作 $a$
- 这一步没有立刻到终点，所以即时奖励 $r = -0.01$
- 下一状态 $s'$ 变成“靠近终点的格子”
- 在这个 $s'$ 里，最好的后续动作还是继续 `right`，假设它的价值是 `1.0`
- 如果 $\gamma = 0.9$，那当前这个动作的大致目标值就是 `-0.01 + 0.9 * 1.0 = 0.89`
