...
也就是说，`Q_\theta(s, a)` 就是在问：

> 参数为 $\theta$ 的这张网络看来，在状态 $s$ 下执行动作 $a$ 值多少钱。
...
- $y$：TD target，也就是本次监督信号
- $r$：当前这一步拿到的奖励
- $\gamma$：折扣因子
- $s'$：下一状态
- $a'$：下一状态里可选的动作
- $\theta^-$：目标网络参数，而不是当前正在更新的参数

如果放到这个仓库的 `LineWorld` 例子里：

- 当前在位置 `3`
- 执行动作 `right`
- 拿到即时奖励 `-0.02`
- 下一状态变成位置 `4`
- 目标网络认为位置 `4` 里最好的动作价值是 `0.95`
- 如果 $\gamma = 0.95$，那 target 大约就是 `-0.02 + 0.95 * 0.95 = 0.8825`
...
- $L(\theta)$：当前 batch 的损失
- $Q_\theta(s, a)$：当前网络给出的预测值
- $y$：上面那条 target

如果当前网络只预测出了 `0.70`，但 target 是 `0.8825`，那损失就是在逼它把 `0.70` 往 `0.8825` 靠。
